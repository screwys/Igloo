...
func TestXProfileMediaReaderLimitsResultsToOriginalPosts(t *testing.T) {
	d := openWritableTestDB(t)
	now := time.Now().UTC()
	items := []model.FeedItem{
		{TweetID: "first_original", SourceHandle: "sample_artist", AuthorHandle: "sample_artist", BodyText: "First original", MediaJSON: `[{"url":"https://cdn.example/first.jpg","type":"photo"}]`, PublishedAt: &now, FetchedAt: now, ContentHash: "first"},
	...
	if len(media) != 2 || media[0].TweetID != "original_media" || media[1].TweetID != "first_original" {
