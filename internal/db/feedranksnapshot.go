...
func (db *DB) ListSnapshotPage(snapshotAt int64, afterPos int, limit int) ([]SnapshotPageItem, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&p.Item.BodyText, &p.Item.ArticleTitle, &p.Item.PollJSON, &p.Item.CommunityNote, &p.Item.Lang,
			...
			&p.Item.QuoteBodyText, &p.Item.QuoteArticleTitle, &p.Item.QuotePollJSON, &p.Item.QuoteCommunityNote, &p.Item.QuoteLang,
