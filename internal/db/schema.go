...
func EnsureSchemaWithOptions(conn *sql.DB, opts EnsureSchemaOptions) error {
	...
	indexes := []string{
		...
		"CREATE INDEX IF NOT EXISTS idx_feed_seen_at ON feed_seen(seen_at, tweet_id)",
		...
		"CREATE INDEX IF NOT EXISTS idx_feed_rank_snapshot_history_tweet ON feed_rank_snapshot_history(tweet_id, computed_at)",
