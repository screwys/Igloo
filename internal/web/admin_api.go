...
func (s *Server) handleConfigImport(w http.ResponseWriter, r *http.Request) {
	...
	if isZipPayload(data) {
		...
		result, restoredMedia, err := s.importFullExportZip(data, userID, replace)
		if err != nil {
...
func (s *Server) importFullExportZip(data []byte, userID string, replace bool) (db.ImportResult, int, error) {
	...
	result, err := s.db.ImportConfig(cfg, userID, replace)
	if err != nil {
		return result, 0, err
	}
	restored, err := s.restoreFullExportBookmarkMedia(zr)
	return result, restored, err
}

func (s *Server) restoreFullExportBookmarkMedia(zr *zip.Reader) (int, error) {
	restored := 0
	for _, f := range zr.File {
		bookmarkID, fileName, ok := fullExportBookmarkMediaEntry(f.Name)
		if !ok || f.FileInfo().IsDir() {
			continue
		}
		destRel := filepath.Join("media", "imported", "bookmarks", bookmarkID, fileName)
		destAbs := filepath.Join(s.cfg.DataDir, destRel)
		if !pathWithinDir(s.cfg.DataDir, destAbs) {
		...
		if err := s.recordRestoredBookmarkMedia(bookmarkID, destRel, mediaIndex, mediaType, wrote); err != nil {
			return restored, err
		}
		restored++
	}
...
func fullExportBookmarkMediaEntry(name string) (string, string, bool) {
...
func (s *Server) recordRestoredBookmarkMedia(videoID, relPath string, mediaIndex int, mediaType string, fileSize int64) error {
	return s.db.WithWrite(func(tx *sql.Tx) error {
		if _, err := tx.Exec(`
		...
		if mediaIndex == 0 {
			...
			`, filepath.Join(s.cfg.DataDir, relPath), videoID); err != nil {
				return err
			}
		}
		return nil
