...
object IglooMigrations {
        ...
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                ...
                CREATE TABLE IF NOT EXISTS android_sync_generations (
                    ...
                    items_imported_seq INTEGER NOT NULL DEFAULT 0
                )
                """.trimIndent(),
            )
            db.execSQL(
    ...
    val MIGRATION_35_36 = object : Migration(35, 36) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                """
                CREATE INDEX IF NOT EXISTS idx_android_sync_assets_generation_state_path
                ON android_sync_assets(generation_id, state, local_path)
                """.trimIndent(),
            )
        }
    }
    ...
    val MIGRATION_36_37 = object : Migration(36, 37) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE android_sync_generations ADD COLUMN items_imported_seq INTEGER NOT NULL DEFAULT 0")
        }
    }
